...
class PathConfig:
    ...
    def __init__(self):
        ...
        # Podcast output is always the same folder as reports so backup,
        # email attachment lookup, and archiving all resolve from one place.
        # Override with PODCAST_DIR env var only if you explicitly want separation.
        self.podcast_dir = self._resolve_path(
            os.getenv("PODCAST_DIR", os.getenv("REPORT_OUTPUT_DIR", "results/reports"))
        )
        ...
        # DEA_VS_MODE=persistent  → survives restarts, remembers papers across runs
        # DEA_VS_MODE=memory      → resets each run (original behaviour)
        self.vector_store_mode = os.getenv("DEA_VS_MODE", "persistent")
        self.vector_store_path = self._resolve_path(
            os.getenv("DEA_VS_PATH", "results/vector_db")
        )
        self.duplicate_threshold = float(os.getenv("DEA_DUP_THRESHOLD", "0.95"))
    ...
    def _log_configuration(self) -> None:
        ...
        logger.info(f"Podcast Directory:        {self.podcast_dir}")
        logger.info(f"Vector DB Mode:           {self.vector_store_mode}")
        logger.info(f"Vector DB Path:           {self.vector_store_path}")
    ...
    def get_vector_store_mode(self) -> str:
        """Return 'persistent' (default) or 'memory'."""
        return self.vector_store_mode

    def get_vector_store_path(self) -> Path:
        """Return on-disk path for persistent vector DB."""
